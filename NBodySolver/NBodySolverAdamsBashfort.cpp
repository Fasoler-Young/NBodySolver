...
//#pragma omp parallel for
		for (int body = 0; body < count; body++) {
			prev_dv[prev_dv.size() - 1][body] = get_data()->calculate_total_force(vector3(), body) / mass[body];
			for (size_t i = 0; i < rank; i++) {
				//for (size_t j = 0; j <= i; j++) {
					dv[body] += prev_dv[rank - i - 1][body] * B[rank - 1][i];
				//}
			}
			velosites[body] = get_data()->Kahan_sum(velosites[body], dv[body] * *dt, &corr_v[body]);
		}
//#pragma omp parallel for
		for (int body = 0; body < count; body++) {
			prev_dc[prev_dc.size() - 1][body] = velosites[body];
			for (size_t i = 0; i < rank; i++) {
				//for (size_t j = 0; j <= i; j++) {
					dcoord[body] += prev_dc[rank - i - 1][body] * B[rank - 1][i];
				//}
			}
			coord[body] = get_data()->Kahan_sum(coord[body], dcoord[body] * *dt, &corr_c[body]);
		}
